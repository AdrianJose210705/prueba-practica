...
class JuegoIntruso:
    def __init__(self):
        ...
        # Configuración de ventana estandarizada según requisitos
    ...
    def inicializar_botones(self):
        ...
        # Disposición espacial de botones
        ...
        self.boton_reiniciar.rect.centery = self.ALTO // 2 + 40
        self.boton_salir1.rect.centerx = self.ANCHO // 2
        self.boton_salir1.rect.centery = self.ALTO // 2 + 110

        # Preparar los textos de los botones
        for b, txt in [
    ...
    def aplicar_penalizacion(self):
        ...
        pts_penal, t_penal = penalizaciones.get(
            self.dificultad_actual, (5, 0.5)
        )
    ...
    def ejecutar(self):
        ...
        while ejecutando:
            ...
            )  # Paso del tiempo exacto a 60 FPS
            ...
            if self.estado_juego == "MENU":
                ...
                if clic and self.boton_inicio.rect.collidepoint(mouse_pos):
                    self.estado_juego = "DIFICULTAD"

            # --- ESTADO: DIFICULTAD ---
            elif self.estado_juego == "DIFICULTAD":
                titulo = self.fuente_titulo.render(
                    "Dificultad", True, self.BLANCO
                )
                self.pantalla.blit(
                    titulo, (self.ANCHO // 2 - titulo.get_width() // 2, 150)
                )
            ...
            # --- ESTADO: JUGANDO ---
                ...
                # Descuento exacto por delta-time
                self.tiempo_restante -= dt

                txt_tiempo = self.fuente_texto.render(
                    f"Tiempo: {max(0, int(self.tiempo_restante))}s",
                    True,
                    self.BLANCO,
                )
                ...
                for r in range(filas):
                    for c in range(columnas):
                        ...
                        rect = pygame.Rect(
                            x, y, ancho_celda - 5, alto_celda - 5
                        )
                ...
                # Clic fallido en zona vacía fuera de celdas
                if (
                    clic
            ...
            # --- ESTADO: GAME OVER ---
                ...
                txt_fin = self.fuente_titulo.render(
                    "JUEGO TERMINADO", True, self.ROJO
                )
                ...
                self.pantalla.blit(
                    txt_fin,
                    (self.ANCHO // 2 - txt_fin.get_width() // 2, self.ALTO // 2 - 140),
                )
                self.pantalla.blit(
                    txt_mensaje,
                    (
